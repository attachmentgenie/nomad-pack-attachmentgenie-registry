...
```
nomad-pack run mysql --var message="Hola Mundo!"
```
...
balance the Consul service `mysql-service`, which is the default value
...
- `consul_service_tags` (list of string) - The consul service name for the
  mysql application
- `consul_service_name` (string) - The consul service name for the mysql
  application
