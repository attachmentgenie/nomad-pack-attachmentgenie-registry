...
```
nomad-pack run mailpit --var message="Hola Mundo!"
```
...
balance the Consul service `mailpit-service`, which is the default value
...
- `consul_service_tags` (list of string) - The consul service name for the
  mailpit application
- `consul_service_name` (string) - The consul service name for the mailpit
  application
