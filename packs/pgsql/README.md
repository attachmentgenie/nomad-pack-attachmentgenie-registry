...
```
nomad-pack run pgsql --var message="Hola Mundo!"
```
...
balance the Consul service `pgsql-service`, which is the default value
...
- `consul_service_tags` (list of string) - The consul service name for the
  pgsql application
- `consul_service_name` (string) - The consul service name for the pgsql
  application
